...
WGAN을 적응형 가중치 계획(adaptive weighting scheme)로 발전된 WGAN을 사용합니다. 마지막으로, 독립적인 batch normalization layer를 사용하여 input distributions에
...
입니다. 이 공식은 gradients가 1보다 작거나 같은 Lipschitz를 더욱 잘 반영하고, 1보다 큰 부분에 대해서반 페널티를 부여합니다. 더욱 중요한 점은, 이 구조에서는 weight $$\lambda$$의 조정을 위해 적응형 가중치 계획(adaptive weight scheme)를 채택하였으며, 이 방법을 통해 gradient가 원하는 간격[1.001, 1.05] 내에 위치하도록 할 수 있는 적절한 가중치(weight) 값을 선택할 수 있습니다.
...
하지만 fib 1.(b)를 보시면, $$G_X$$의 input이 $$X$$의 sample에서 사용되고,, $$G'_X$$의 input은 $$X'$$의 sample에서 사용되는 것을 볼 수 있습니다. 이들은 서로 다른 **분포 특성(distribution characteristics)**을 가질 수 있으며, 이는 $$G_X$$와 $$G'_X$$가 각각의 input들에 더 잘 적응할 수 있도록 만들어줍니다.
...
![dpe_table2](/assets/img/dpe/fig4.png)

&nbsp;&nbsp;&nbsp;&nbsp; fig4를 보면, iBN을 사용하는 2-way GAN에 비해 iBN을 사용하지 않은 2-way GAN은 특징 분포를 포착하는데 실패한 모습을 볼 수 있습니다.
