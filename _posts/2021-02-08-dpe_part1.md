...
DPE 는 Unpaired learning을 수행하기 때문에, 사용자가 생각하는 "보좋은" 이미지 dataset만 준비하면 해당 dataset의 특성을 학습하여 input image를 "좋은" 이미지로
...
DPE의 구조는 위 fig1과 같은 모습입니다. Source domain인 X는 원본 이미지이며, target domain Y는 원하는 enhancement 특징을 가지는 image들입니다.
Fig1의 (a)의 경우에는 1-way GAN의 모습을 보여줍니다. 원본 이미지  X는 Generator를 통해 y의 특징을 갖는 Y' 으로 변환되고, Discriminaotr Dy는 target domain
y와 generator로 생성된  sample x'를 구분합니다.
